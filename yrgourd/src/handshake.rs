...
pub fn initiate(
    ...
    let static_shared = (responder.q * initiator_static.d).compress().to_bytes();
    yr.mix(b"static-shared", &static_shared);

    // Generate a hedged commitment scalar and commitment point.
    let k = yr.hedge(&mut rng, &[initiator_static.d.as_bytes()], 10_000, |clone| {
    ...
    s_enc.copy_from_slice(((initiator_static.d * r) + k).as_bytes());
...
pub fn accept(
    ...
    // Calculate the static shared secret and mix it into the protocol.
    let static_shared = (static_pub.q * responder_static.d).compress().to_bytes();
    yr.mix(b"static-shared", &static_shared);
    ...
    // Calculate the ephemeral shared secret and mix it into the protocol.
    let ephemeral_shared = (static_pub.q * responder_ephemeral.d).compress().to_bytes();
    yr.mix(b"responder-ephemeral-shared", &ephemeral_shared);

    // Generate a hedged commitment scalar and commitment point.
    let k = yr.hedge(&mut rng, &[responder_static.d.as_bytes()], 10_000, |clone| {
        Some(Scalar::from_bytes_mod_order_wide(&clone.derive_array(b"commitment-scalar")))
    });
    let i = RistrettoPoint::mul_base(&k);
    ...
    s_enc.copy_from_slice(((responder_static.d * r) + k).as_bytes());
...
pub fn finalize(
    ...
    // Calculate the ephemeral shared secret and mix it into the protocol.
    let ephemeral_shared = (initiator_static.d * responder_ephemeral.q).compress().to_bytes();
    yr.mix(b"responder-ephemeral-shared", &ephemeral_shared);
